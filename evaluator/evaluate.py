...
# ------------------------------------------------------------------
# 5. EFFECTIVENESS -- routing relevance: apakah case_history_search mengarahkan
#    ke tipe kasus (maintenance vs komplain) yang benar sesuai domain query.
#    Ini proxy tool-level untuk efektivitas routing yang idealnya dilakukan
#    orchestrator -- lihat juga bukti routing end-to-end sungguhan pada
#    Bab V.4 laporan (orchestrator berhasil merutekan ke Maintenance+Finance
#    Agent sesuai query).
# ------------------------------------------------------------------
def eval_effectiveness(k: int = 3):
    case_tool = CaseSearchTool()
    test_cases = [
        ("mesin CNC rusak karena tool wear", "maintenance_case"),
        ("conveyor mati mendadak saat produksi", "maintenance_case"),
        ("barang pesanan belum sampai ke pelanggan", "complaint_ticket"),
        ("produk retak saat diterima pelanggan", "complaint_ticket"),
    ]

    correct = 0
    details = []
    for query, expected_type in test_cases:
        result = case_tool.run(query=query, n_results=k)
        # hitung jenis mana yang dominan di top-k hasil (berdasarkan label [type, ...] di awal tiap entri)
        maint_count = result.count("maintenance_case")
        ticket_count = result.count("complaint_ticket")
        dominant = "maintenance_case" if maint_count >= ticket_count else "complaint_ticket"
        is_correct = dominant == expected_type
        correct += int(is_correct)
        details.append({"query": query, "expected": expected_type, "dominant_result": dominant, "correct": is_correct})

    return {
        "metric": "effectiveness",
        "component": "case_history_search (routing relevance proxy)",
        "routing_accuracy_pct": round(correct / len(test_cases) * 100, 1),
        "n_test_queries": len(test_cases),
        "detail": details,
    }
